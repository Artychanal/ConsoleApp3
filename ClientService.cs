...
namespace ConsoleApp3.solution.services.impl;
...
public class ClientService : IService<Client>
{
    ...
    public void Save(Client client)
    {
        ...
        {
            Client foundClient = clients[clients.FindIndex(cli => cli.Id == client.Id)];
            foundClient.Name = client.Name;
            foundClient.Surname = client.Surname;
        }
        else
        {
            client.Id = clients.Count > 0 ? clients.Max(cli => cli.Id) + 1 : 1;
            clients.Add(client);
        }
        SaveToFile(clients);
    }
    ...
    public List<Client> GetAll(SortField sortField, SortOrder sortOrder)
    {
        ...
        {
            case SortField.Name:
                clients = sortOrder == SortOrder.Asc ? clients.OrderBy(c => c.Name).ToList() : clients.OrderByDescending(c => c.Name).ToList();
                break;
            case SortField.Surname:
                clients = sortOrder == SortOrder.Asc ? clients.OrderBy(c => c.Surname).ToList() : clients.OrderByDescending(c => c.Surname).ToList();
                break;
        }
        return clients;
    }
    ...
    {
        List<Client> clients = LoadFromFile();
        return clients.Where(cli => query.All(k => cli.Keywords.Contains(k))).ToList();
    }
